...
@section('body')
    <div class="md:flex md:justify-center md:gap-10 md:items-center ">
        <div class="md:w-6/12 p-5">
            <img src="{{ asset('assets/img/login.jpg') }}" alt="Imagen login de usuarios">
        </div>

        <div class="md:w-4/12 bg-white p-6 rounded-lg shadow-xl">
            <form class="space-y-4 md:space-y-6" action="{{ route('login') }}" method="POST" autocomplete="off">
                @csrf
                <!-- email -->
                <div>
                ...
                <div class="flex items-start">
                    <div class="flex items-center h-5">
                        <input id="remember" aria-describedby="terms" type="checkbox" name="remember"
                            class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-primary-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-primary-600 dark:ring-offset-gray-800">
                    </div>
                    <div class="ml-3 text-sm">
                        <label for="terms"
                            class="font-light text-gray-500 dark:text-gray-300">{{ __('Mantener mi sesión abierta') }}</label>
                    </div>
                </div>
                <button type="submit"
                ...
                <p class="text-sm font-light text-gray-500 dark:text-gray-400 md:text-left text-center">
                    ...
                    <a href="{{ route('register') }}" class="font-medium text-blue-700 hover:underline dark:text-blue-500">
                        {{ __('Únete a nosotros') }}
                    </a>
                </p>
            </form>
